CREATE KEYSPACE containers WITH replication = {'class':'SimpleStrategy', 'replication_factor' : 3};
CREATE TABLE container(id int PRIMARY KEY, container_size int, container_type text, quantity int, yard text);
CREATE TABLE bookings(booking_ref_id int PRIMARY KEY, container_size int, container_type text, origin text, destination text, quantity int, time_stamp timestamp);
